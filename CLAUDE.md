...
- **[app/Auth.php](app/Auth.php)** — session auth against `users.password_hash` (`password_hash`/`password_verify`), regenerates id on login/logout. **Roles:** `isAdmin()` (role `ผู้ดูแลระบบ`), `requireAdmin()` (members get bounced to `/my`), `homeUrl()` (admins → `/admin`, members → `/my`; used for post-login redirect). Login also gates on account `status` (`pending`/`approved`/`suspended`).
- **[app/ResearchFormTrait.php](app/ResearchFormTrait.php)** — `buildResearchData()` + `storeUploads()` shared by `AdminController` and `MemberController` so both submit paths stay identical; the caller passes which statuses are allowed.
...
- **Controllers** ([app/controllers/](app/controllers/)) — `PublicController` (home/search/detail/download), `AuthController` (login/logout + public self-registration), `AdminController` (`/admin/*`, admin only: dashboard/manage/submit/categories/users), `MemberController` (`/my/*`, teachers & students: submit + manage **their own** research only, cannot self-publish — statuses limited to `แบบร่าง`/`รอตรวจสอบ`), `AccountController` (shared `/account` change-password for any signed-in user). They read `$_GET`/`$_POST`, call the Repository, and either `App::render(...)` or `redirect()`.
- **Views** ([views/](views/)) — plain PHP templates. `layout.php` picks a chrome by `$section`: public (`partials/public_header` + `public_footer`), admin (`partials/admin_shell` = sidebar + top bar), member (`partials/member_shell` = top-bar nav), or `bare` (login/register). `views/submit.php` and `views/account.php` are shared between the admin and member areas (submit is area-aware via `$area`/`$statuses`). Inline styles are ported verbatim from the prototype for pixel fidelity; the design **tokens** (`--bg`, `--primary`, `--c1..--c5`, light/dark) live once in [assets/app.css](assets/app.css) on `.app-root[data-theme]`.

### Key behaviours to preserve

- **Publish gating:** only `status = 'เผยแพร่'` research appears on the public site (home/search/detail); an admin or the record's owner (`created_by`) can view an unpublished record's detail, and locked chapter files download only for admin-or-owner. Enforced in `Repository::search()`/`latestPublished()` and `PublicController::detail()`/`download()`.
- **Only admins publish:** members submit as `แบบร่าง`/`รอตรวจสอบ`; the server coerces any other status they POST. Admins move records to `เผยแพร่`/`ไม่เผยแพร่`.
